...
function listOrder() {
  ...
  wallet.getBalance().then(function(balance) {
    ...
    $.get("http://127.0.0.1:7330/order/list/1", function(data, status) {
  ...
  wallet.getBalance().then(function(balance) {
    ...
    node.rpc.execute({method:'cp.list', params:[]}).then( cps => {
      ...
      for (i = 0; i < cps.length; i++) {
        ...
        wdiv.appendChild(el);
        el.onmouseup = function(ev) {
          wdb.rpc.execute({method:'token.user', params:[
            cp.cid,       //游戏编号
            '10000007'    //游戏内玩家编号
          ]}).then(token => {
            //延迟显示新的厂商列表
            ...
            setTimeout(()=>{
              ...
              location.href = `http://localhost:7330/game/${ts}`;
            }, 1000);
          });
          ev.stopPropagation();
          return false;
...
function formatSales(wdb, cid) {
  ...
  wallet.getBalance().then(function(balance) {
    ...
    //发起针对指定厂商的拍卖交易列表查询
    node.rpc.execute({method:'prop.list.market', params:[
      cid,        //厂商编码
    ]}).then( orders => {
      ...
      for (i = 0; i < orders.length; i++) {
        ...
        el.onmouseup = function(ev) {
          ...
          wdb.rpc.execute({method:'prop.buy', params:[
            order.pid,  //拍卖物品编码
            np,         //新的竞拍价格
          ]}).then(tx => {
            setTimeout(()=>{
